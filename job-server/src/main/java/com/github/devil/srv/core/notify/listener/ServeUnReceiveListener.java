...
public class ServeUnReceiveListener implements Listener<ServeUnReceiveEvent> {
    ...
    public void onEvent(ServeUnReceiveEvent event) {
        ...
        Message message = Message.builder().title(Constants.SERVER_DOWN_TITLE).build();
        ...
        if (Objects.equals(event.getServerHost(), MainAkServer.getCurrentHost())){
            ...
            synchronized (ServeUnReceiveListener.class) {
                // 本机服务正常
                if (MainAkServer.isNormal()) {

                    // 需要进行一次自检,确保本机服务正常
                    try {
                        ServerHolder.ask(MainAkServer.getCurrentHost());
